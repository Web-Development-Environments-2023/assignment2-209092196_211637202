...
function animate() {
  ...
  AlienShots.forEach((shot, shidx) => {
    if (
      shot.point.y + shot.raduis >= canv.height ||
      (shot.point.y + shot.raduis >= player.point.y &&
        shot.point.x + shot.raduis >= player.point.x &&
        shot.point.x - shot.raduis <= player.point.x + player.width)
    ) {
      setTimeout(() => {
        if (
          shot.point.y + shot.raduis >= player.point.y &&
  ...
  aliensGrid.forEach((grid) => {
    ...
    grid.aliens.forEach((ali, idx) => {
      ...
      missles.forEach((mis, indx) => {
        if (
          mis.point.y - mis.raduis <= ali.point.y + ali.height &&
          mis.point.x + mis.raduis >= ali.point.x &&
          mis.point.x - mis.raduis <= ali.point.x + ali.width
        ) {
          setTimeout(() => {
